;;; init-org.el --- Org-mode config for Windows -*- lexical-binding: t -*-
;;; Commentary:
...
;; Ensure pdf-tools is loaded (Windows-compatible setup)
(require-package 'pdf-tools)

;; Windows-specific pdf-tools setup
(when (eq system-type 'windows-nt)
  ;; Try to find pdfinfo executable on Windows
  (let ((pdfinfo-paths '("C:/Program Files/Git/usr/bin/pdfinfo.exe"
                        "C:/msys64/mingw64/bin/pdfinfo.exe"
                        "C:/ProgramData/chocolatey/bin/pdfinfo.exe"
                        "pdfinfo.exe")))
    (dolist (path pdfinfo-paths)
      (when (file-exists-p path)
        (setq pdf-info-program path)
        (return)))))

;; Install pdf-tools (may need manual compilation on Windows)
(condition-case nil
    (pdf-tools-install t)
  (error
   (message "PDF-Tools installation failed. Please install manually on Windows. See: https://github.com/vedang/pdf-tools")))
...
;; Use Windows-compatible paths for org files
(setq org-capture-templates
      `(("t" "Todo" entry (file ,(expand-file-name "inbox.org" org-directory))
        ...
        ("n" "Note" entry (file ,(expand-file-name "inbox.org" org-directory))
         "* %? \n  %i\n  %a" :empty-lines 1)
        ("m" "Meeting" entry (file ,(expand-file-name "inbox.org" org-directory))
         "* MEETING: %? :meeting:\n  SCHEDULED: %t\n\n** Attendees:\n\n** Notes:\n\n** Action Items:\n  - [ ] "
     :empty-lines 1)))
